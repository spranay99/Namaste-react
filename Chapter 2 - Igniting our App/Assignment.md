...
A: `Parcel/Webpack` is type of a web application bundler used for development and productions purposes.
It offers blazing fast performance utilizing multicore processing, and requires zero configuration. Parcel can take any type of file as an entry point, but an HTML or JavaScript file is a good place to start.

### Parcel Features:

- HMR (Hot Module Replacement) - parcel keeps track of file changes via file watcher algorithm and renders the changes in the files. It is made with C++
- Minification
- Cleaning our code
...
- `Image optimization` - Optimizes the assets in our project such as image, videos etc, so that it can minimize the http request thus increasing overall performance.
- `Caching while development` - Stores information about the project, so that when it rebuilds, it can quickly build without actually reparsing it from start.

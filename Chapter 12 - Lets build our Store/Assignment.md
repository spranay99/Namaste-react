...
A: Both usecontext and redux are used to solve `props drilling`, a problem faced while passing props between components.
...
A. A dispatcher is a `function` that dispatches actions to the store. In Redux, actions are used to describe changes to the state, and dispatching an action is the way to trigger those changes.
...
A: A reducer is a `pure function` in Redux that takes the `current state` of your application and an `action`, and returns a `new state` based on that `action`.
...
A: In Redux Toolkit, a `slice` is a piece of the state that is managed by a single set of actions and reducer.

## Q: Explain selector.

A: A `selector` is a pure `function` that takes the current `state` of your application and returns a derived value based on that state.
...
A: The `createSlice` function is used to create a store slice, a piece of the store.
